...
```{r}
map1_2 <- tm_shape(msoas) + # Call area data object
    tm_polygons("total_second_dose", fill.scale = tm_scale(values = "viridis"), fill.legend = tm_legend(title = "Frequency")) + # Which column to plot and plot using colour blind friendly colours and edit legend to 'Frequency'
    tm_title("Second dose uptake") + # Adds title
    tm_scalebar(position = c("right", "bottom")) # Add scale bar
map1_2 # Print
...
map1_4 <- tm_shape(msoas) + # Call area data object
        ...
        tm_title("Second dose uptake") + # Adds titlee
        tm_scalebar(position = c("right", "bottom")) # Add scale bar
map1_4 # Print
